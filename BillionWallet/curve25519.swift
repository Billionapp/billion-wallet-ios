...
public class Curve25519 {
    ...
    class Utility {
        ...
        static func unpack_8(_ str: String) -> [UInt8] {
            ...
            regex.enumerateMatches(in: str, options: [], range: NSMakeRange(0, str.characters.count)) { match, flags, stop in
    ...
    private func mul(_ a: [UInt32], _ b: [UInt32]) -> [UInt32] {
        ...
        c = (b[0] * a[1]) + (b[1] * a[0]) + (b[2] * 19 * a[9]) + (b[3] * 19 * a[8]) + (b[4] * 19 * a[7]) + (b[5] * 19 * a[6]) + (b[6] * 19 * a[5]) + (b[7] * 19 * a[4]) + (b[8] * 19 * a[3]) + (b[9] * 19 * a[2]) + (c >> 26)
        r[1] = c & 0x1ffffff
        
        c = (b[0] * a[2]) + (b[1] * 2 * a[1]) + (b[2] * a[0]) + (b[3] * 38 * a[9]) + (b[4] * 19 * a[8]) + (b[5] * 38 * a[7]) + (b[6] * 19 * a[6]) + (b[7] * 38 * a[5]) + (b[8] * 19 * a[4]) + (b[9] * 38 * a[3]) + (c >> 25)
        r[2] = c & 0x3ffffff
        
        c = (b[0] * a[3]) + (b[1] * a[2]) + (b[2] * a[1]) + (b[3] * a[0]) + (b[4] * 19 * a[9]) + (b[5] * 19 * a[8]) + (b[6] * 19 * a[7]) + (b[7] * 19 * a[6]) + (b[8] * 19 * a[5]) + (b[9] * 19 * a[4]) + (c >> 26)
        r[3] = c & 0x1ffffff
        
        c = (b[0] * a[4]) + (b[1] * 2 * a[3]) + (b[2] * a[2]) + (b[3] * 2 * a[1]) + (b[4] * a[0]) + (b[5] * 38 * a[9]) + (b[6] * 19 * a[8]) + (b[7] * 38 * a[7]) + (b[8] * 19 * a[6]) + (b[9] * 38 * a[5]) + (c >> 25)
        r[4] = c & 0x3ffffff
        
        c = (b[0] * a[5]) + (b[1] * a[4]) + (b[2] * a[3]) + (b[3] * a[2]) + (b[4] * a[1]) + (b[5] * a[0]) + (b[6] * 19 * a[9]) + (b[7] * 19 * a[8]) + (b[8] * 19 * a[7]) + (b[9] * 19 * a[6]) + (c >> 26)
        r[5] = c & 0x1ffffff
        
        c = (b[0] * a[6]) + (b[1] * 2 * a[5]) + (b[2] * a[4]) + (b[3] * 2 * a[3]) + (b[4] * a[2]) + (b[5] * 2 * a[1]) + (b[6] * a[0]) + (b[7] * 38 * a[9]) + (b[8] * 19 * a[8]) + (b[9] * 38 * a[7]) + (c >> 25)
        r[6] = c & 0x3ffffff
        
        c = (b[0] * a[7]) + (b[1] * a[6]) + (b[2] * a[5]) + (b[3] * a[4]) + (b[4] * a[3]) + (b[5] * a[2]) + (b[6] * a[1]) + (b[7] * a[0]) + (b[8] * 19 * a[9]) + (b[9] * 19 * a[8]) + (c >> 26)
        r[7] = c & 0x1ffffff
        
        c = (b[0] * a[8]) + (b[1] * 2 * a[7]) + (b[2] * a[6]) + (b[3] * 2 * a[5]) + (b[4] * a[4]) + (b[5] * 2 * a[3]) + (b[6] * a[2]) + (b[7] * 2 * a[1]) + (b[8]    * a[0]) + (b[9] * 38 * a[9]) + (c >> 25)
        r[8] = c & 0x3ffffff
        
        c = (b[0] * a[9]) + (b[1] * a[8]) + (b[2] * a[7]) + (b[3] * a[6]) + (b[4] * a[5]) + (b[5] * a[4]) + (b[6] * a[3]) + (b[7] * a[2]) + (b[8] * a[1]) + (b[9]    * a[0]) + (c >> 26)
        r[9] = c & 0x1ffffff
