...
class ECIESTests: XCTestCase {
    ...
    func testAll() {
        let config = Crypto().genECIESConfig()
        ...
        ecies_a.setUp(priv: aliceKey!, pub: bobPub)
        ecies_b.setUp(priv: bobKey!, pub: alicePub)
        ...
        for message in texts {
            ...
            let msgCypher_ab = ecies_a.encrypt(msgData)
            let msgCypher_ba = ecies_b.encrypt(msgData)
            ...
            let msgDecypher_ab = ecies_b.decrypt(msgCypher_ab.c, mac: msgCypher_ab.d)
            let msgDecypher_ba = ecies_a.decrypt(msgCypher_ba.c, mac: msgCypher_ba.d)
    ...
    func testSpeed() {
        let configA = Crypto().genECIESConfig()
        ...
        self.measure {
            for _ in 0..<10 {
                let msgCypher_ab = ecies_a.encrypt(msgData)
                let _ = ecies_a.decrypt(msgCypher_ab.c, mac: msgCypher_ab.d)
            }
        }
    }
    
    func testSpeedLocalCrypto() {
        let configA = LocalCrypto().genECIESConfig()
        let ecies_a = ECIES(with: configA)
        let bobPub = ECIES.privToPub(bobKey!)
        ecies_a.setUp(priv: aliceKey!, pub: bobPub)
        let text = texts[0]
